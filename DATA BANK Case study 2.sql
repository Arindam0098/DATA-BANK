...
FROM regions, customer_nodes, customer_transactions  -- ← add 's' here
WHERE regions.region_id = customer_nodes.region_id 
AND customer_nodes.customer_id = customer_transactions.customer_id  -- ← and here
...
JOIN customer_transactions USING(customer_id)
GROUP BY region_name
...
from customer_transactions)) as avg_deposite_amount 
from customer_transactions
...
with transactions_count_per_month_cte as
...
select txn_month,count(distinct customer_id) as customer_count from transactions_count_per_month_cte
